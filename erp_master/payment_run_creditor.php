...
$stmt = $pdo->prepare("SELECT kontonr, bezeichnung FROM bankkonten WHERE aktiv = ?");
$stmt->execute([1]);
$bankkonten = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
    ...
    try {
        ...
        if (isset($_POST['confirm']) && $_POST['confirm'] === '1') {
            ...
            foreach ($offene as $posten) {
                ...
                $insert = $pdo->prepare("
                    INSERT INTO zahllauefe_kreditor (offener_posten_id, kreditorennr, betrag, zahlungsdatum, user_id)
                    VALUES (?, ?, ?, ?, ?)
                ");
                $insert->execute([
                    $posten['id'],
            ...
            // Kreditoren aktualisieren
          $stmt = $pdo->prepare("
           UPDATE kreditoren
            SET offener_betrag = (
            SELECT COALESCE(SUM(betrag - bezahlt), 0)
...
");
            ...
            $aktualisierteKreditoren = array_unique(array_column($offene, 'kreditorennr'));
            foreach ($aktualisierteKreditoren as $kreditor) {
            $stmt->execute([$kreditor, $kreditor]);
            }
