...
CREATE TABLE `journal` (
  ...
  `benutzer_id` int NOT NULL,
  `zahlungsbedingungen` ENUM('14', '30', '90') DEFAULT '30'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
...
CREATE TABLE `journalzeile` (
  ...
  `status` enum('A','G','S') NOT NULL, -- möglicherweise in Journal verschieben, da Allgemeiner Status (Dilemma, wenn man nur eine Zeile löschen möchte)
  `vorgang` enum('H','D','K','A','L') NOT NULL,
  `buchungstext` varchar(100) DEFAULT NULL,
  `buchungsdatum` date NOT NULL,
  `belegdatum` date NOT NULL
...
CREATE TABLE if not exists `debitoren` (
    `id` INT NOT NULL,
    `debitorennr` INT NOT NULL UNIQUE,
    `bezeichnung` VARCHAR(100) NOT NULL,
    `zahlungsbedingungen` ENUM('14', '30', '90') DEFAULT '30',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;

-- Tabelle für Kreditoren
CREATE TABLE IF NOT EXISTS `kreditoren` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `kreditorennr` INT NOT NULL UNIQUE,
    `bezeichnung` VARCHAR(100) NOT NULL,
    `zahlungsbedingungen` ENUM('14', '30', '90') DEFAULT '30',
    PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
...
-- ===== Erweiterungen gemäß Projektstruktur =====
-- Neue Tabelle für Umsatzsteuerkennzeichen
CREATE TABLE IF NOT EXISTS `steuerkennzeichen` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `code` VARCHAR(20) NOT NULL,
  `bezeichnung` VARCHAR(255) NOT NULL
);

-- Neue Users-Tabelle für modernes Login
...
-- Ergänzungen für kontenstamm (Zahlungsbedingungen & offene Posten)
ALTER TABLE `kontenstamm`
  ADD COLUMN `open_amount` DECIMAL(11,2) DEFAULT 0.00;

-- Ergänzungen für transactions (Umsatzsteuer-ID)
-- Hinweis: Nur ausführen, wenn Tabelle transactions existiert
-- ALTER TABLE transactions
--   ADD COLUMN ust_id INT,
--   ADD FOREIGN KEY (ust_id) REFERENCES steuerkennzeichen(id);
