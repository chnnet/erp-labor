...
class PaymentGuard
{
    ...
    public static function isLocked(int $journalId): bool
    {
        ...
        $stmt1 = $pdo->prepare("SELECT COUNT(*) FROM zahlungseingaenge_debitor WHERE journal_id = ?");
        ...
        $stmt2 = $pdo->prepare("SELECT COUNT(*) FROM zahllauefe_kreditor WHERE FIND_IN_SET(?, journal_id)");
